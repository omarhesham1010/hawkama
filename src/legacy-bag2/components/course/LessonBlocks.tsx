    ...
    <div>
      {block.subtitle && (
        <p className="mb-5 text-center">
          <span className="chip bg-goldLegacy-500/15 text-goldLegacy-600 dark:text-goldLegacy-300 text-base font-bold px-4 py-1.5">
            « {block.subtitle} »
          </span>
        </p>
      )}
      <div className="grid gap-4 lg:grid-cols-3">
        {block.columns.map((col, colIndex) => {
          ...
          const headerBg = gold
            ? 'bg-gradient-to-l from-goldLegacy-600 to-goldLegacy-500'
            : 'bg-gradient-to-l from-greenLegacy-700 to-greenLegacy-600';
          const dot = gold ? 'bg-goldLegacy-500' : 'bg-greenLegacy-600 dark:bg-greenLegacy-400';
          const topLine = gold ? 'bg-goldLegacy-500' : 'bg-greenLegacy-600';
          return (
            <div
              key={col.index}
            ...
            >
              ...
              <div className={`flex items-center gap-3 px-4 py-3.5 text-white ${headerBg}`}>
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white/95 font-extrabold tabular text-greenLegacy-800">
                  {col.index}
                </span>
  ...
  switch (block.kind) {
    ...
    case 'callout': {
      const tones = {
        ...
        gold: 'bg-goldLegacy-500/10 border-goldLegacy-500/30',
        contrast:
          'bg-gradient-to-l from-greenLegacy-800 to-greenLegacy-600 text-white border-greenLegacy-600 dark:from-greenLegacy-900 dark:to-greenLegacy-700',
      } as const;
      const isContrast = block.tone === 'contrast';
      return (
        <div className={`rounded-2xl border p-6 shadow-card ${tones[block.tone]}`}>
          <div className="flex items-start gap-3">
            <span className={isContrast ? 'text-goldLegacy-400' : 'text-brand'}>
            ...
            <div>
              ...
              <p className={`text-[17px] leading-relaxed ${isContrast ? 'text-greenLegacy-50' : 'text-ink-soft'}`}>
                {block.text}
              </p>
            </div>
          </div>
