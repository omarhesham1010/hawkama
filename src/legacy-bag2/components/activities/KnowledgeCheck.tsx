  ...
  if (finished) {
    ...
    return (
      <div data-quiz-result="true" className="relative flex h-full animate-scale-in flex-col items-center justify-center gap-3 text-center">
        ...
        <div className={`grid h-20 w-20 place-items-center rounded-full ${passed ? 'bg-tealLegacy-500/15 text-tealLegacy-600' : 'bg-goldLegacy-500/15 text-goldLegacy-600'}`}>
        ...
        <p className={`rounded-xl px-4 py-2 text-base font-semibold ${passed ? 'bg-tealLegacy-500/10 text-tealLegacy-700' : 'bg-goldLegacy-500/10 text-goldLegacy-700'}`}>
          {passed
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="mt-2 grid min-h-0 flex-1 grid-cols-2 grid-rows-2 gap-2.5">
        {q.options.map((opt, i) => {
          ...
          if (answered) {
            if (isRight) cls = 'border-tealLegacy-500/60 bg-tealLegacy-500/10 text-tealLegacy-800';
          ...
          return (
            ...
            >
              <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl text-base font-extrabold tabular ${answered && isRight ? 'bg-tealLegacy-500 text-white' : answered && isSel ? 'bg-rose-500 text-white' : 'bg-surface-3 text-ink-muted'}`}>
      ...
      <div className="mt-2 flex shrink-0 items-stretch gap-3">
        {answered ? (
          <div className={`flex flex-1 items-start gap-2 rounded-2xl border px-4 py-3 text-base font-semibold leading-snug ${isCorrect ? 'border-tealLegacy-500/40 bg-tealLegacy-500/[0.06] text-ink' : 'border-rose-400/40 bg-rose-500/[0.06] text-ink'}`}>
            <Icon name={isCorrect ? 'check' : 'alert'} className={`mt-0.5 h-5 w-5 shrink-0 ${isCorrect ? 'text-tealLegacy-600' : 'text-rose-500'}`} />
