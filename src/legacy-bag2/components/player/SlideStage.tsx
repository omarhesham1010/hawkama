...
import { FlipCardActivity } from '../../../components/activities/FlipCardActivity';
...
import { POSE_SRC, POSE_SRC_BAG2, type NasserPose } from '../../../components/character/Nasser';
...
const ACCENTS = [
  { border: 'border-greenLegacy-400/70', tile: 'bg-greenLegacy-500/18 text-greenLegacy-700' },
  { border: 'border-tealLegacy-400/70', tile: 'bg-tealLegacy-500/18 text-tealLegacy-700' },
  { border: 'border-sky-400/70', tile: 'bg-sky-500/18 text-sky-700' },
  { border: 'border-goldLegacy-400/80', tile: 'bg-goldLegacy-500/22 text-goldLegacy-700' },
];

const ACTIVE_BOX = 'border-greenLegacy-600 bg-gradient-to-br from-greenLegacy-500 to-greenLegacy-700 shadow-glow';
...
function ContextOrnament({
  ...
  return (
    ...
    >
      <span
        className="absolute inset-0 m-auto rounded-full border border-dashed border-greenLegacy-700/25"
        style={{ height: size.primaryBox + 44, width: size.primaryBox + 44 }}
      />
      <span
        className="absolute inset-0 m-auto grid place-items-center rounded-full border-[3px] border-white bg-white/95 shadow-card-lg ring-2 ring-greenLegacy-600/25"
        style={{ height: size.primaryBox, width: size.primaryBox, fontSize: size.primaryText }}
      >
        {primary}
        <span className="absolute -right-1 -top-1 h-4 w-4 rounded-full border-[3px] border-white bg-goldLegacy-400" />
      </span>
      <span
        className="absolute bottom-0 left-0 grid -rotate-[8deg] place-items-center rounded-full border-2 border-white bg-greenLegacy-50/95 shadow-card ring-1 ring-greenLegacy-600/20"
  ...
  switch (unit.t) {
      ...
      return (
        <div className={`${shell} flex items-start gap-4 p-5`}>
          ...
          <div>
            ...
            <p className={`text-[20px] font-semibold leading-relaxed ${active ? 'text-greenLegacy-50' : 'text-ink-soft'}`}>
              {unit.text}
            </p>
          </div>
        </div>
      ...
      return (
        <div className={`${shell} flex min-h-[64px] items-center gap-3 p-3.5`}>
          ...
          <div className="min-w-0">
            ...
            {unit.text && (
              <p className={`text-[18px] font-semibold leading-relaxed ${active ? 'text-greenLegacy-50' : 'text-ink-soft'}`}>
                {unit.text}
              </p>
            )}
          </div>
    ...
    case 'callout': {
      ...
      const cls = isContrast
        ? 'border-greenLegacy-600 bg-gradient-to-l from-greenLegacy-800 to-greenLegacy-600 text-white'
        : active
          ? `${ACTIVE_BOX} text-white`
          : unit.tone === 'gold'
            ? 'border-goldLegacy-400/70 bg-goldLegacy-500/10'
            : 'border-tealLegacy-400/70 bg-tealLegacy-500/10';
      const titleCls = isContrast || active ? 'text-white' : 'text-ink';
      const textCls = isContrast || active ? 'text-greenLegacy-50' : 'text-ink-soft';
      return (
...
function SideVisual({ slide, alt }: { slide: Slide; alt: boolean }) {
  ...
  return (
    <div
      ...
      } border-2 border-greenLegacy-500/35 shadow-card-lg ${
        alt
          ? 'bg-gradient-to-tr from-tealLegacy-600/28 via-greenLegacy-500/24 to-greenLegacy-700/30'
          : 'bg-gradient-to-br from-greenLegacy-500/30 via-tealLegacy-500/22 to-greenLegacy-700/28'
      }`}
    >
      <div className="absolute -right-10 -top-10 h-44 w-44 rounded-full bg-greenLegacy-400/40 blur-3xl" />
      <div className="absolute -left-10 -bottom-10 h-44 w-44 rounded-full bg-goldLegacy-400/30 blur-3xl" />
      <div
  ...
  return (
    <span
      className={`grid h-10 w-10 shrink-0 place-items-center rounded-full text-[15px] font-extrabold text-white shadow-card ring-4 ring-surface transition-transform ${
        active ? 'scale-110 bg-gradient-to-br from-greenLegacy-500 to-greenLegacy-700' : 'bg-gradient-to-br from-greenLegacy-400 to-tealLegacy-600'
      }`}
    >
      {toArabicDigits(n)}
    </span>
  ...
  return (
    <span
      className={`grid h-9 w-9 shrink-0 rotate-45 place-items-center rounded-xl shadow-card ring-4 ring-surface transition-transform ${
        active ? 'scale-110 bg-gradient-to-br from-greenLegacy-500 to-greenLegacy-700' : 'bg-gradient-to-br from-tealLegacy-400 to-sky-500'
      }`}
    >
...
function BeatSlide({ slide, spoken, showDialogue }: { slide: Slide; spoken: number; showDialogue: boolean }) {
  ...
  const compactVisual = (
    <div className="relative flex h-32 min-w-[220px] flex-1 items-end justify-between overflow-visible rounded-3xl border-2 border-greenLegacy-500/30 bg-gradient-to-l from-greenLegacy-500/18 via-tealLegacy-500/12 to-goldLegacy-500/14 px-5 shadow-card">
  ...
  const flowRows = bodyBeats.map(({ b, i }) => {
    ...
    const connector = <span className={`h-0.5 w-4 shrink-0 rounded ${active ? 'bg-greenLegacy-600' : 'bg-brand/40'}`} />;
    const content = (
  ...
  const verticalFlow = (
    <div className="relative flex min-h-0 flex-1 flex-col justify-center gap-3">
      {/* connecting spine */}
      <span
        className={`absolute bottom-7 top-7 z-0 w-1 rounded-full bg-gradient-to-b from-greenLegacy-500/60 via-tealLegacy-500/55 to-goldLegacy-500/55 ${
  ...
  const topDownFlow = (
    <div className="relative grid min-h-0 flex-1 grid-cols-2 content-center gap-3.5">
      <span className="absolute left-6 right-6 top-1/2 z-0 h-1 -translate-y-1/2 rounded-full bg-gradient-to-l from-greenLegacy-500/55 via-tealLegacy-500/45 to-goldLegacy-500/50" />
      {flowRows}
    </div>
  );
  ...
  return (
    <StorySlideShell slide={slide} spoken={spoken} showDialogue={showDialogue}>
      ...
      <span className={`grid h-14 w-14 shrink-0 place-items-center bg-gradient-to-br from-greenLegacy-500 to-tealLegacy-600 p-2 text-white shadow-card ${alt ? 'rounded-full' : 'rounded-2xl'}`}>
...
const PPT_ACCENTS: Record<NonNullable<PptCard['tone']>, string> = {
  green: 'border-greenLegacy-600/30 bg-white',
  gold: 'border-goldLegacy-500/45 bg-goldLegacy-50/80',
    ...
    <div className="absolute inset-0 z-20 grid place-items-start pt-[104px]" aria-live="polite">
      ...
      <div
        className={`pointer-events-auto relative mx-auto w-full max-w-[720px] animate-scale-in overflow-visible rounded-2xl border-2 shadow-card-lg transition-colors duration-500 ${
          revealed
            ? 'border-greenLegacy-700 bg-gradient-to-br from-greenLegacy-700 to-greenLegacy-600 text-white'
            : 'border-goldLegacy-500/60 bg-white text-ink'
        }`}
      >
        <div className={`flex items-center gap-2 border-b px-5 py-2.5 ${revealed ? 'border-white/20 bg-white/10' : 'border-goldLegacy-500/25 bg-goldLegacy-50'}`}>
          <span className="text-[20px]">{revealed ? '✅' : phase === 'asking' ? '🎙️' : '🤔'}</span>
          <span className={`text-[13px] font-extrabold ${revealed ? 'text-white' : 'text-greenLegacy-800'}`}>
            سؤال ناصر التفاعلي
          </span>
        </div>
        <div className="px-6 py-5 text-center">
          ...
          <p className={`mt-2 text-[16px] font-bold leading-relaxed ${revealed ? 'text-greenLegacy-50' : 'text-ink-soft'}`}>
        ...
        {!revealed ? (
          <div className="flex justify-center pb-5">
            <button
              ...
              disabled={!ready}
              onClick={onReveal}
              className={`rounded-lg bg-greenLegacy-700 px-6 py-3 text-[16px] font-extrabold text-white shadow-card transition hover:bg-greenLegacy-800 disabled:cursor-not-allowed disabled:bg-white disabled:text-ink-muted disabled:shadow-none ${
          ...
          <div className="flex justify-center pb-4">
            <button
              ...
              onClick={onDismiss}
              className="text-[13px] font-bold text-greenLegacy-50/80 underline underline-offset-2 disabled:cursor-not-allowed disabled:opacity-40"
            >
...
function IntroMotionScene({
  ...
  const introLayerSrcs = isEmergencyCourse
    ? [
        '/assets/visual-library/intro-emergency-preparedness-shield-legacy.webp',
          ...
          return (
            <div
              ...
              className={`${cardWidth} shrink-0 rounded-2xl border px-2.5 py-3 text-center shadow-sm backdrop-blur-md transition-all duration-[900ms] ease-out ${
                ...
                active
                  ? 'z-10 -translate-y-1 scale-[1.04] border-goldLegacy-500/55 bg-greenLegacy-800/94 text-white shadow-card-lg'
                  : 'border-greenLegacy-700/20 bg-white/90 text-greenLegacy-900'
              }`}
              style={{
                order: visualOrder,
            ...
            >
              ...
              <p className={`mt-1 text-[11px] font-extrabold leading-snug ${active ? 'text-greenLegacy-50' : 'text-ink'}`}>{pillar.detail}</p>
            </div>
          );
        })}
        </div>
      ...
      <div className="relative z-10 flex h-full min-h-0 flex-col justify-between px-6 py-5 text-right">
        <div className="mr-auto w-[49%] animate-fade-up">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <span className="rounded-full border border-greenLegacy-700/18 bg-white/78 px-4 py-1.5 text-[14px] font-extrabold text-greenLegacy-800 shadow-sm backdrop-blur-sm">
              تحت إشراف المدرب ناصر
            </span>
            <span className="rounded-full border border-goldLegacy-500/25 bg-goldLegacy-50/78 px-4 py-1.5 text-[14px] font-extrabold text-goldLegacy-700 shadow-sm backdrop-blur-sm">
              بداية الرحلة
            </span>
          </div>

          <p className="mb-2 text-[20px] font-extrabold text-greenLegacy-800">{slide.ppt?.courseName}</p>
          {slide.ppt?.subtitle && (
            <p className="mb-2 w-fit rounded-lg bg-greenLegacy-700 px-4 py-1.5 text-[18px] font-extrabold text-white shadow-card">
              {slide.ppt.subtitle}
            </p>
          )}
  ...
  return (
    <div className="relative h-full min-h-0 overflow-visible">
      ...
      <div className="relative z-10 flex h-full min-h-0 flex-col px-5 py-5">
            ...
            return (
              <div
                ...
                className={`relative flex items-center gap-3 overflow-visible rounded-[18px] border px-3 py-2 text-right shadow-[0_16px_30px_rgb(24_82_55_/_0.10)] backdrop-blur-md transition-all duration-700 ${
                  ...
                  active
                    ? 'z-20 translate-x-2 scale-[1.02] border-goldLegacy-500/55 bg-gradient-to-br from-greenLegacy-800 via-greenLegacy-700 to-tealLegacy-700 text-white shadow-card-lg'
                    : 'border-greenLegacy-700/14 bg-white/88 text-brand-strong'
                }`}
                style={{ minHeight: 76 }}
              >
                <div className={`absolute inset-x-0 top-0 h-1.5 ${active ? 'bg-goldLegacy-500' : 'bg-greenLegacy-700/35'}`} />
                <span className={`grid h-11 w-11 shrink-0 place-items-center rounded-full border text-[22px] font-black tabular-nums ${
                    active ? 'border-white/30 bg-white/16 text-white' : 'border-greenLegacy-700/16 bg-white/88 text-greenLegacy-800'
                  }`}>
                ...
                <span
                  className={`grid h-[60px] w-[68px] shrink-0 place-items-center rounded-[20px] border bg-white/90 p-2 transition-all duration-700 ${
                    active ? 'motion-layer-focus border-white/40 shadow-[0_16px_24px_rgb(0_0_0_/_0.12)]' : 'border-greenLegacy-700/12 drop-shadow-[0_14px_20px_rgb(24_82_55_/_0.10)]'
                  }`}
        ...
        <div className="pointer-events-none absolute bottom-5 right-6 left-6 h-px bg-gradient-to-l from-transparent via-greenLegacy-700/16 to-transparent" />
      </div>
    </div>
  );
}
...
function pptGeneratedVisualLayersFor(text: string, fallback?: string) {
  ...
  if (hasAny(text, ['معايير التصعيد', 'تسلسل تصعيد', 'أخضر', 'أصفر', 'أحمر', 'أسود', 'برتقالي', 'ربط العتبات بالإجراءات'])) {
    add(variantOf(['/assets/visual-library/emergency-escalation-levels-legacy.webp', '/assets/visual-library/emergency-escalation-levels-legacy.webp'], text));
  }
  if (hasAny(text, ['OODA', 'Observe لاحظ', 'Orient توجّه', 'Decide قرّر', 'Act نفّذ'])) {
    add(variantOf(['/assets/visual-library/emergency-ooda-loop-legacy.webp', '/assets/visual-library/emergency-ooda-loop-legacy.webp'], text));
  }
  if (hasAny(text, ['CERC', 'ما قبل الأزمة', 'المرحلة الأولية', 'مرحلة الصيانة', 'مرحلة الحل', 'مرحلة التقييم'])) {
    add(variantOf(['/assets/visual-library/emergency-cerc-timeline-legacy.webp', '/assets/visual-library/emergency-cerc-timeline-legacy.webp'], text));
  }
  if (hasAny(text, ['PESTLE', 'المسح الأفقي', 'السياسة والتشريع', 'القطاع الخاص'])) {
    add(variantOf(['/assets/visual-library/emergency-pestle-wheel-legacy.webp', '/assets/visual-library/emergency-pestle-wheel-legacy.webp'], text));
  }
  if (hasAny(text, ['المسح الرأسي', 'المستوى الكلي', 'Macro', 'المستوى المتوسط', 'Meso', 'المستوى الجزئي', 'Micro'])) {
    add(variantOf(['/assets/visual-library/emergency-macro-meso-micro-legacy.webp', '/assets/visual-library/emergency-macro-meso-micro-legacy.webp'], text));
  }
  if (hasAny(text, ['دلفي', 'مسح الخبراء'])) {
    add(variantOf(['/assets/visual-library/emergency-delphi-method-legacy.webp', '/assets/visual-library/emergency-delphi-method-legacy.webp'], text));
  }
  if (hasAny(text, ['عجلة المستقبل', 'تحليل التأثير المتبادل'])) {
    add(variantOf(['/assets/visual-library/emergency-futures-wheel-legacy.webp', '/assets/visual-library/emergency-futures-wheel-legacy.webp'], text));
  }
  if (hasAny(text, ['الخطر Hazard', 'التعرض Exposure', 'الهشاشة Vulnerability', 'أخطار متتالية', 'أخطار متزامنة', 'أخطار متزايدة', 'متعددة الأخطار'])) {
    add(variantOf(['/assets/visual-library/emergency-hazard-triangle-legacy.webp', '/assets/visual-library/emergency-hazard-triangle-legacy.webp'], text));
  }
  if (hasAny(text, ['مصفوفة السيناريوهات', 'حالات عدم اليقين', 'القوى الدافعة', 'الروايات', 'سؤال محوري'])) {
    add(variantOf(['/assets/visual-library/emergency-scenario-matrix-legacy.webp', '/assets/visual-library/emergency-scenario-matrix-legacy.webp'], text));
  }
  if (hasAny(text, ['مؤشرات استباقية', 'Leading', 'مؤشرات متأخرة', 'Lagging', 'مؤشرات كمية', 'Quantitative', 'مؤشرات نوعية', 'Qualitative'])) {
    add(variantOf(['/assets/visual-library/emergency-leading-lagging-legacy.webp', '/assets/visual-library/emergency-leading-lagging-legacy.webp'], text));
  }
  if (hasAny(text, ['الترصد القائم على المؤشرات', 'الترصد القائم على الأحداث'])) {
    add(variantOf(['/assets/visual-library/emergency-surveillance-modes-legacy.webp', '/assets/visual-library/emergency-surveillance-modes-legacy.webp'], text));
  }
  if (hasAny(text, ['الفئة A', 'الفئة B', 'الفئة C', 'ABC'])) {
    add(variantOf(['/assets/visual-library/emergency-abc-inventory-legacy.webp', '/assets/visual-library/emergency-abc-inventory-legacy.webp'], text));
  }
  if (hasAny(text, ['PDCA', 'دورة PDCA'])) {
    add(variantOf(['/assets/visual-library/emergency-pdca-cycle-legacy.webp', '/assets/visual-library/emergency-pdca-cycle-legacy.webp'], text));
  }
  if (hasAny(text, ['القوة والاهتمام', 'راقب', 'أبقهم مطلعين', 'أبقهم راضين', 'أدرهم عن كثب'])) {
    add(variantOf(['/assets/visual-library/emergency-power-interest-matrix-legacy.webp', '/assets/visual-library/emergency-power-interest-matrix-legacy.webp'], text));
  }
  if (hasAny(text, ['كن الأول', 'كن دقيقًا', 'كن ذا مصداقية', 'عبّر عن التعاطف', 'شجّع على العمل', 'أظهر الاحترام'])) {
    add(variantOf(['/assets/visual-library/emergency-crisis-comm-principles-legacy.webp', '/assets/visual-library/emergency-crisis-comm-principles-legacy.webp'], text));
  }
  if (hasAny(text, ['معايير SMART', 'SMART', 'SMART-ER'])) {
    add(variantOf(['/assets/visual-library/emergency-smart-criteria-legacy.webp', '/assets/visual-library/emergency-smart-criteria-legacy.webp'], text));
  }
  if (hasAny(text, ['السيناريو الأفضل', 'السيناريو الأسوأ', 'السيناريو الأكثر ترجيحًا'])) {
    add(variantOf(['/assets/visual-library/emergency-scenario-branches-legacy.webp', '/assets/visual-library/emergency-scenario-branches-legacy.webp'], text));
  }
  ...
  if (hasAny(text, ['مصفوفة RACI', 'RACI', 'المسؤول عن التنفيذ', 'المساءَل', 'يجب استشارته', 'يجب إعلامه'])) {
    add(variantOf(['/assets/visual-library/emergency-raci-matrix-legacy.webp', '/assets/visual-library/emergency-raci-matrix-legacy.webp', '/assets/visual-library/emergency-raci-matrix-legacy.webp'], text));
  }
  if (hasAny(text, ['ديناميكيات الأزمات', 'أنواع الأزمات', 'الحدث Event', 'الطارئ Emergency', 'الأزمة Crisis', 'الكارثة Disaster'])) {
    add('/assets/visual-library/emergency-crisis-terms-legacy.webp');
  }
  if (hasAny(text, ['القيادة أثناء الأزمات', 'سمات القيادة', 'الهدوء الظاهر', 'الحسم في القرارات', 'التعاطف الإنساني', 'التواجد الميداني'])) {
    add(variantOf(['/assets/visual-library/emergency-leadership-traits-legacy.webp', '/assets/visual-library/emergency-leadership-traits-legacy.webp', '/assets/visual-library/emergency-leadership-traits-legacy.webp'], text));
  }
  if (hasAny(text, ['ICS', 'EOC', 'قائد الحادث', 'مركز عمليات الطوارئ', 'مركز القيادة', 'مركز قيادة', 'التنسيق أثناء الطوارئ'])) {
    add(variantOf(['/assets/visual-library/emergency-command-center-legacy.webp', '/assets/visual-library/emergency-command-center-legacy.webp', '/assets/visual-library/emergency-command-center-legacy.webp'], text));
  }
  if (hasAny(text, ['الإطار الاستراتيجي', 'الاستعداد للطوارئ', 'خطط الطوارئ', 'الجاهزية المؤسسية', 'التكامل بين الجهات', 'السياسات التشغيلية', 'تفعيل السياسات'])) {
    add('/assets/visual-library/emergency-strategic-framework-legacy.webp');
  }
  if (hasAny(text, ['استمرارية الأعمال', 'التعافي', 'الموارد الحيوية', 'العمليات الحرجة'])) {
    add(variantOf(['/assets/visual-library/emergency-continuity-shield-legacy.webp', '/assets/visual-library/emergency-continuity-shield-legacy.webp', '/assets/visual-library/emergency-continuity-shield-legacy.webp'], text));
  }
  if (
    hasAny(text, [
      'التواصل',
    ...
    add(variantOf(['/assets/visual-library/emergency-crisis-communication-legacy.webp', '/assets/visual-library/emergency-crisis-communication-legacy.webp', '/assets/visual-library/emergency-crisis-communication-legacy.webp'], text));
  }
  if (hasAny(text, ['اتخاذ القرار', 'شلل التحليل', 'تحت الضغط'])) {
    add(variantOf(['/assets/visual-library/emergency-decision-pressure-legacy.webp', '/assets/visual-library/emergency-decision-pressure-legacy.webp', '/assets/visual-library/emergency-decision-pressure-legacy.webp'], text));
  }
  if (hasAny(text, ['المسح الاستباقي', 'استشراف', 'التخطيط بالسيناريوهات'])) {
    add('/assets/visual-library/emergency-proactive-scanning-legacy.webp');
  }
  if (hasAny(text, ['خريطة المخاطر', 'خرائط المخاطر', 'مصفوفة تفاعل الأخطار'])) {
    add(variantOf(['/assets/visual-library/emergency-risk-matrix-legacy.webp', '/assets/visual-library/emergency-risk-matrix-legacy.webp', '/assets/visual-library/emergency-risk-matrix-legacy.webp'], text));
  }
  if (hasAny(text, ['الترصد', 'الإنذار المبكر', 'حصن', 'مستويات العتبة'])) {
    add(variantOf(['/assets/visual-library/emergency-surveillance-radar-legacy.webp', '/assets/visual-library/emergency-surveillance-radar-legacy.webp', '/assets/visual-library/emergency-surveillance-radar-legacy.webp'], text));
  }
  if (hasAny(text, ['سلسلة التوريد', 'اللوجستيات', 'اللوجستية', 'المخزون', 'نوبكو', 'EOQ', 'الموردين', 'مرونة سلسلة'])) {
    add(variantOf(['/assets/visual-library/emergency-supply-chain-legacy.webp', '/assets/visual-library/emergency-supply-chain-legacy.webp', '/assets/visual-library/emergency-supply-chain-legacy.webp'], text));
  }
  if (hasAny(text, ['مراجعة ما بعد الحدث', 'AAR', 'الدروس المستفادة', 'خطة التحسين', 'خطة تحسين'])) {
    add(variantOf(['/assets/visual-library/emergency-after-action-review-legacy.webp', '/assets/visual-library/emergency-after-action-review-legacy.webp', '/assets/visual-library/emergency-after-action-review-legacy.webp'], text));
  }
  if (hasAny(text, ['مؤشرات الأداء', 'مؤشرات الجاهزية', 'مؤشرات الاستجابة', 'مؤشرات النتائج', 'مؤشرات التحسين', 'KPI', 'لوحة معلومات', 'Dashboard', 'خط الأساس'])) {
    add(variantOf(['/assets/visual-library/emergency-kpi-dashboard-legacy.webp', '/assets/visual-library/emergency-kpi-dashboard-legacy.webp', '/assets/visual-library/emergency-kpi-dashboard-legacy.webp'], text));
  }
  if (hasAny(text, ['أصحاب المصلحة', 'Stakeholders'])) {
    add(variantOf(['/assets/visual-library/emergency-stakeholder-network-legacy.webp', '/assets/visual-library/emergency-stakeholder-network-legacy.webp', '/assets/visual-library/emergency-stakeholder-network-legacy.webp'], text));
  }
  if (hasAny(text, ['الموارد البشرية', 'تنسيق الموارد', 'تنسيق المعلومات', 'تدفق المعلومات', 'فرق الاستجابة', 'الكوادر'])) {
    add(variantOf(['/assets/visual-library/emergency-response-team-legacy.webp', '/assets/visual-library/emergency-response-team-legacy.webp', '/assets/visual-library/emergency-response-team-legacy.webp'], text));
  }
  ...
  if (layers.length === 1) {
    if (layers[0].includes('emergency-command')) add('/assets/visual-library/emergency-strategic-framework-legacy.webp');
    else if (layers[0].includes('emergency')) add('/assets/visual-library/emergency-command-center-legacy.webp');
...
const SHARED_BRAND_ICON_POOL = [...new Set([
  '/assets/visual-library/icon-shield-check-legacy.webp',
  '/assets/visual-library/icon-institution-board-legacy.webp',
  '/assets/visual-library/icon-target-alignment-legacy.webp',
  '/assets/visual-library/icon-announcement-speaker-legacy.webp',
  '/assets/visual-library/icon-performance-chart-legacy.webp',
  '/assets/visual-library/icon-control-grid-legacy.webp',
  '/assets/visual-library/icon-alert-bell-legacy.webp',
  '/assets/visual-library/icon-network-nodes-legacy.webp',
  '/assets/visual-library/icon-response-vehicle-legacy.webp',
  '/assets/visual-library/icon-policy-document-legacy.webp',
  '/assets/visual-library/icon-strategy-cube-legacy.webp',
  '/assets/visual-library/icon-kpi-dashboard-legacy.webp',
  '/assets/visual-library/icon-file-report-legacy.webp',
  '/assets/visual-library/icon-ethics-balance-legacy.webp',
  '/assets/visual-library/icon-direction-sign-legacy.webp',
  '/assets/visual-library/icon-discussion-bubble-legacy.webp',
  '/assets/visual-library/icon-shield-check-legacy.webp',
  '/assets/visual-library/icon-checklist-action-legacy.webp',
  '/assets/visual-library/icon-institution-board-legacy.webp',
  '/assets/visual-library/icon-target-alignment-legacy.webp',
  '/assets/visual-library/icon-control-grid-legacy.webp',
  '/assets/visual-library/icon-alert-bell-legacy.webp',
  ...
  '/assets/visual-library/icon-response-vehicle-legacy.webp',
  '/assets/visual-library/icon-policy-document-legacy.webp',
  '/assets/visual-library/icon-search-plus-legacy.webp',
  '/assets/visual-library/icon-strategy-cube-legacy.webp',
  '/assets/visual-library/icon-kpi-dashboard-legacy.webp',
  '/assets/visual-library/icon-file-report-legacy.webp',
  '/assets/visual-library/icon-direction-sign-legacy.webp',
...
function PptTitle({ slide, showVisual = true }: { slide: Slide; showVisual?: boolean }) {
  ...
  return (
    <div className="mb-4 text-center">
      {slide.ppt?.eyebrow && (
        <p className="mb-1 text-[17px] font-extrabold text-goldLegacy-600">{slide.ppt.eyebrow}</p>
      )}
      ...
      {slide.ppt?.subtitle && slide.ppt?.courseName && (
        <p className="mx-auto mb-2 w-fit rounded-full border border-greenLegacy-700/20 bg-greenLegacy-50 px-5 py-1 text-[18px] font-extrabold text-greenLegacy-800">
          {slide.ppt.subtitle}
        </p>
      )}
      <h2 className={`inline-flex items-center justify-center text-[36px] font-extrabold leading-tight text-brand-strong ${showVisual ? 'gap-3' : ''}`}>
        {showVisual && (
          <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl border border-greenLegacy-700/16 bg-white/80 p-1.5 shadow-sm">
            {brandIcon ? (
...
function PptCardView({
  ...
  const passiveShell =
    tone === 'gold'
      ? 'border-goldLegacy-500/18 bg-[linear-gradient(145deg,rgb(255_255_255_/_0.96),rgb(249_244_226_/_0.78)_58%,rgb(255_255_255_/_0.9))]'
      : tone === 'blue'
        ...
        : tone === 'gray'
          ...
          : 'border-greenLegacy-700/14 bg-[linear-gradient(145deg,rgb(255_255_255_/_0.96),rgb(233_246_239_/_0.74)_58%,rgb(255_255_255_/_0.9))]';
  const activeShell = active
    ? 'scale-[1.025] border-goldLegacy-500/45 bg-[linear-gradient(145deg,rgb(17_92_58),rgb(18_119_96)_62%,rgb(197_162_80))] text-white shadow-card-lg'
    : passiveShell;
  const titleTone = active ? 'text-white' : 'text-brand-strong';
  const bodyTone = active ? 'text-greenLegacy-50' : 'text-ink';
  const tileTone = active
    ? 'border-white/35 bg-white/16 text-white shadow-card'
    : tone === 'gold'
      ? 'border-goldLegacy-500/22 bg-white/90'
      : tone === 'blue'
        ? 'border-sky-500/20 bg-white/90'
        : 'border-greenLegacy-700/14 bg-white/90';
  const accentTone = active ? 'bg-goldLegacy-300' : tone === 'gold' ? 'bg-goldLegacy-500' : tone === 'blue' ? 'bg-tealLegacy-500' : 'bg-greenLegacy-700';
      ...
      <div className={`${padClass} relative z-10 flex min-h-0 flex-col items-center justify-center`}>
        <div className={`${level === 'micro' ? 'mb-1 gap-1' : 'mb-2.5 gap-2'} relative flex w-full flex-col items-center`}>
          <span className={`relative grid ${visualSize} shrink-0 place-items-center border ${tileTone} shadow-[0_18px_34px_rgb(24_82_55_/_0.14)] [border-radius:30px_18px_28px_18px]`}>
            <span className="pointer-events-none absolute -left-1.5 -top-1.5 h-6 w-6 rounded-full border border-white/80 bg-goldLegacy-400/85 shadow-sm" aria-hidden="true" />
            {brandIcon && (
          ...
          {card.index && (
            <span className={`absolute right-3 top-1 grid ${indexSize} shrink-0 place-items-center rounded-full font-extrabold tabular ${active ? 'bg-white/20 text-white ring-2 ring-white/25' : 'bg-greenLegacy-700 text-white shadow-sm'}`}>
              {card.index}
            </span>
          )}
        ...
        {showAnswerDetail ? (
          <div className={`mt-1 rounded-2xl border p-2 ${active ? 'border-white/25 bg-white/15' : 'border-greenLegacy-700/20 bg-white/54'}`}>
          ...
          <ul className={`${level === 'micro' ? 'mt-1 space-y-1' : 'mt-1.5 space-y-1.5'} w-full`}>
            {card.bullets.map((bullet, i) => (
              <li key={i} className={`${bulletClass} flex justify-center gap-2 text-center font-bold ${bodyTone}`}>
                <span className={`${level === 'micro' ? 'mt-1 h-1.5 w-1.5' : 'mt-1.5 h-2 w-2'} shrink-0 rounded-full ${active ? 'bg-goldLegacy-300' : 'bg-goldLegacy-500'}`} />
                <span>{bullet}</span>
              </li>
            ))}
          </ul>
...
function slideVisualPool(slide: Slide, cards: PptCard[]) {
  ...
  const fallbackVisualPool = isEmergencySlide
    ? ['/assets/visual-library/emergency-command-center-legacy.webp', '/assets/visual-library/emergency-strategic-framework-legacy.webp', '/assets/visual-library/emergency-stakeholder-network-legacy.webp', '/assets/visual-library/emergency-kpi-dashboard-legacy.webp']
...
function PptMotionVisualScene({
  ...
  const chapterAccent = slide.id.startsWith('ch3') ? 'from-tealLegacy-50/80' : slide.id.startsWith('ch2') ? 'from-goldLegacy-50/80' : 'from-greenLegacy-50/80';
  ...
  return (
    <div className="relative min-h-0 flex-1 overflow-visible rounded-[30px]">
      ...
      {showMotionGraphics && <div className="absolute inset-x-[5%] bottom-[2%] h-px bg-gradient-to-r from-transparent via-greenLegacy-700/35 to-transparent" />}
        ...
        return (
          <button
            ...
            className={`absolute ${labelPositions[index % labelPositions.length]} isolate text-right transition-all duration-700 ease-out ${
              usesOpenLabels
                ? titleCardGrid
                  ? titleGridSparse
                    ? 'min-h-[178px] rounded-[32px] border border-greenLegacy-700/10 bg-white/34 px-4 py-4 shadow-[0_18px_38px_rgb(24_82_55_/_0.09)] backdrop-blur-[2px]'
                    : 'min-h-[142px] rounded-[28px] border border-greenLegacy-700/10 bg-white/34 px-3 py-3 shadow-[0_16px_34px_rgb(24_82_55_/_0.08)] backdrop-blur-[2px]'
                  : 'min-h-[104px] rounded-none border-0 bg-transparent px-1 py-1 shadow-none backdrop-blur-0'
                : 'min-h-[118px] rounded-[999px] border border-greenLegacy-700/10 bg-white/45 px-5 py-3 shadow-[0_18px_34px_rgb(24_82_55_/_0.08)] backdrop-blur-sm'
            } ${
              active
                ? usesOpenLabels
                  ? 'z-20 scale-[1.04] text-brand-strong'
                  : 'scale-[1.04] border-goldLegacy-500/50 bg-white/75 text-brand-strong shadow-[0_22px_36px_rgb(24_82_55_/_0.14)]'
          ...
          >
            <span className={`flex items-center justify-between ${titleCardGrid ? 'flex-col gap-2 text-center' : emergencyOpenLabels ? 'gap-3' : usesOpenLabels ? 'gap-6' : 'gap-4'}`}>
              <span className={`relative z-10 min-w-0 flex-1 ${titleCardGrid ? 'order-2 w-full border-t-[4px] border-goldLegacy-500/70 pt-2' : emergencyOpenLabels ? 'border-r-[4px] border-goldLegacy-500/70 pr-3' : usesOpenLabels ? 'border-r-[5px] border-goldLegacy-500/70 pr-4' : ''}`}>
                <span className={`flex items-center font-black leading-tight text-brand-strong drop-shadow-[0_2px_3px_rgb(255_255_255_/_0.95)] ${titleCardGrid ? 'justify-center gap-2' : 'justify-end gap-3'} ${openLabelTitleClass}`}>
                  ...
                  {brandIcon && !titleCardGrid && !emergencyOpenLabels && (
                    <span className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl border border-greenLegacy-700/10 bg-white/78 p-1.5 shadow-sm" aria-hidden="true">
                ...
                <span
                  className={`mt-3 block h-[5px] rounded-full transition-all duration-500 ${
                    active ? 'w-[86%] bg-goldLegacy-500' : 'w-[44%] bg-greenLegacy-700/18'
                  }`}
                  aria-hidden="true"
                />
              </span>
              <span className={`relative z-0 ${titleCardGrid ? 'order-1' : ''} ${openLabelImageClass} shrink-0 rounded-full bg-white/35 ring-1 ring-goldLegacy-500/16 transition-all duration-700 ${active ? 'shadow-[0_14px_28px_rgb(191_155_74_/_0.22)]' : ''}`}>
                {brandIcon && (
                  <img
                    src={brandIcon}
                    alt=""
        ...
        {cards.map((card, index) => {
          ...
          return (
            <div key={index} className="flex items-start">
              {index > 0 && (
                <span className="relative mx-2 mt-8 hidden h-[3px] w-8 shrink-0 overflow-hidden rounded-full bg-greenLegacy-700/14 sm:block" aria-hidden="true">
                  {visibleFor(index - 1) && (
                    <span
              ...
              >
                <span className={`relative grid ${denseEmergencyTimeline ? 'h-[84px] w-[84px]' : isEmergencySlide ? 'h-[104px] w-[104px]' : 'h-[64px] w-[64px]'} shrink-0 place-items-center`}>
                  {active && (
                    <span className="absolute inset-0 rounded-full border-2 border-goldLegacy-400 animate-ring-shockwave" aria-hidden="true" />
                  )}
                  <span
                    className={`absolute inset-0 grid place-items-center rounded-full border-4 text-[22px] font-extrabold tabular shadow-card transition-all duration-500 ${
                      active
                        ? 'animate-glow-cycle border-goldLegacy-400 bg-gradient-to-br from-greenLegacy-700 to-greenLegacy-600 text-white shadow-card-lg'
                        : 'border-white bg-white text-greenLegacy-800 ring-1 ring-greenLegacy-700/16'
                    }`}
                  >
                  {brandIcon ? (
                ...
                <span
                  className={`mt-3 px-4 py-2.5 transition-all duration-700 ${
                    isEmergencySlide
                      ? active
                        ? 'border-b-4 border-goldLegacy-500 text-brand-strong'
                        : 'border-b-4 border-greenLegacy-700/18 text-brand-strong'
                      : active
                        ? 'rounded-[22px] border border-goldLegacy-500/40 bg-white text-brand-strong shadow-card'
                        : 'rounded-[22px] border border-greenLegacy-700/12 bg-white/72 text-brand-strong shadow-[0_14px_30px_rgb(24_82_55_/_0.08)] backdrop-blur-sm'
                  }`}
                >
                  {brandIcon && !isEmergencySlide && (
                    <span className="mx-auto mb-2 grid h-12 w-12 place-items-center rounded-2xl border border-greenLegacy-700/10 bg-white/80 p-1.5 shadow-sm" aria-hidden="true">
                      <img src={brandIcon} alt="" className={`h-full w-full object-contain ${activeVisualClass(active, `${card.title} ${card.text ?? ''}`, index)}`} loading="lazy" decoding="async" />
                    </span>
                  )}
    ...
    <div className={`flex min-h-0 flex-1 flex-col items-center justify-start px-3 ${denseQuadrant ? 'pt-[60px] pb-[120px]' : 'pt-2 pb-[190px]'}`}>
      <div className={`relative grid w-full max-w-[1040px] auto-rows-fr ${cols} ${denseQuadrant ? 'gap-2' : 'gap-5'}`}>
        {cards.map((card, index) => {
          ...
          const toneShell =
            tone === 'gold'
              ? 'border-goldLegacy-500/25 bg-[linear-gradient(150deg,rgb(255_255_255_/_0.97),rgb(249_244_226_/_0.7))]'
              : tone === 'blue'
                ? 'border-sky-500/22 bg-[linear-gradient(150deg,rgb(255_255_255_/_0.97),rgb(232_247_249_/_0.68))]'
                : 'border-greenLegacy-700/18 bg-[linear-gradient(150deg,rgb(255_255_255_/_0.97),rgb(233_246_239_/_0.68))]';
          return (
            <button
              key={index}
              type="button"
              ...
              className={`relative overflow-visible ${denseQuadrant ? 'h-[94px] p-2 text-center' : 'min-h-[134px] p-5 text-right'} ${isEmergencySlide ? '[border-radius:44px_20px_44px_20px]' : 'rounded-[26px]'} border shadow-[0_16px_34px_rgb(24_82_55_/_0.08)] transition-all duration-700 ease-out ${
                ...
                active
                  ? 'scale-[1.02] animate-glow-cycle border-goldLegacy-500/45 bg-[linear-gradient(150deg,rgb(17_92_58),rgb(18_119_96)_62%,rgb(197_162_80))] text-white shadow-card-lg'
                  : toneShell
              }`}
            >
              {active && (
              ...
              <span
                className={`absolute ${denseQuadrant ? 'right-2 top-2 h-6 w-6 text-[10px]' : 'right-4 top-4 h-9 w-9 text-[13px]'} grid place-items-center rounded-full font-extrabold tabular ${
                  active ? 'bg-white/22 text-white ring-2 ring-white/25' : 'bg-greenLegacy-700 text-white'
                }`}
              >
                {card.index ?? index + 1}
              </span>
              ...
              {card.text && (
                <p className={`${denseQuadrant ? 'mt-0.5 line-clamp-2 overflow-hidden px-2 text-center text-[10.5px] leading-snug' : 'mt-3 pe-1 text-[16px] leading-relaxed'} font-bold ${active ? 'text-greenLegacy-50' : 'text-ink'}`}>{card.text}</p>
              )}
              {showDetail && detail && (
                <div className={`mt-2 rounded-2xl border p-2 ${active ? 'border-white/25 bg-white/15' : 'border-greenLegacy-700/20 bg-white/60'}`}>
                  <p className={`text-[12px] font-bold leading-snug ${active ? 'text-greenLegacy-50' : 'text-ink'}`}>{detail}</p>
                </div>
              )}
            </button>
          );
  ...
  return (
    <div className={`flex min-h-0 flex-1 flex-col items-center justify-center gap-5 px-4 ${denseSupporting ? 'pb-[70px]' : 'pb-[65px]'}`}>
      <button
        ...
        } ${focusVisible ? 'animate-glow-cycle' : ''} border-goldLegacy-500/35 bg-[linear-gradient(160deg,rgb(17_92_58),rgb(18_119_96)_62%,rgb(197_162_80))] text-white`}
      >
        {focusVisible && (
          <span
        ...
        {focusCard?.text && <p className={`relative z-10 ${isEmergencySlide ? 'mx-auto max-w-[520px] text-[16px]' : 'text-[16px]'} mt-1.5 font-bold leading-snug text-greenLegacy-50`}>{focusCard.text}</p>}
        {showFocusDetail && focusDetail && (
          <div className="mx-auto mt-3 max-w-[520px] rounded-2xl border border-white/25 bg-white/15 p-2.5">
            <p className="text-[13px] font-bold leading-snug text-greenLegacy-50">{focusDetail}</p>
          </div>
        )}
      </button>
      {supporting.length > 0 && (
            ...
            return (
              <button
                ...
                className={`${denseSupporting ? 'min-w-[128px]' : 'min-w-[170px]'} flex-1 ${isEmergencySlide ? `border-b-4 ${denseSupporting ? 'px-2 py-2' : 'px-4 py-2'} shadow-none` : 'rounded-[22px] border px-4 py-3 shadow-[0_14px_28px_rgb(24_82_55_/_0.07)]'} text-center transition-all duration-500 ${
                  ...
                  active
                    ? isEmergencySlide
                      ? 'animate-glow-cycle scale-[1.03] border-goldLegacy-500 text-brand-strong'
                      : 'animate-glow-cycle scale-[1.03] border-goldLegacy-500/40 bg-white text-brand-strong shadow-card'
                    : isEmergencySlide
                      ? 'border-greenLegacy-700/16 text-brand-strong'
                      : 'border-greenLegacy-700/14 bg-white/75 text-brand-strong'
                }`}
              >
                {brandIcon && (
                  <span className={`mx-auto ${denseSupporting ? 'mb-1 h-10 w-10' : 'mb-2 h-14 w-14'} grid place-items-center rounded-2xl border border-greenLegacy-700/10 bg-white/78 p-1 shadow-sm`} aria-hidden="true">
                    <img src={brandIcon} alt="" className={`h-full w-full object-contain ${activeVisualClass(active, `${card.title} ${card.text ?? ''}`, index)}`} loading="lazy" decoding="async" />
                  </span>
                )}
  ...
  return (
    <StorySlideShell slide={slide} spoken={spoken} showDialogue={showDialogue || Boolean(interactionLine)} dialogueOverride={interactionLine}>
      <div className="flex h-full min-h-0 flex-col px-7 py-3">
        ...
        <div className="mb-3 rounded-lg border-r-8 border-goldLegacy-500 bg-white/95 px-3.5 py-2.5 text-right shadow-sm">
          ...
          {currentCard && (
            <div
              ...
              className={`relative grid h-full min-h-0 grid-cols-[190px_1fr] items-center gap-5 overflow-visible rounded-[34px] border px-6 py-4 text-right shadow-[0_18px_42px_rgb(24_82_55_/_0.10)] transition-all duration-500 ${
                answerVisible
                  ? 'scale-[1.01] border-goldLegacy-500/45 bg-[linear-gradient(145deg,rgb(17_92_58),rgb(18_119_96)_62%,rgb(197_162_80))] text-white'
                  : 'border-greenLegacy-700/14 bg-white/90 text-brand-strong'
              } ${currentCardVisible ? PPT_REVEAL_ANIMS[currentStep % PPT_REVEAL_ANIMS.length] : 'pointer-events-none opacity-0'}`}
            >
              <span className={`pointer-events-none absolute right-5 top-5 h-3 w-14 rounded-full ${answerVisible ? 'bg-goldLegacy-300' : 'bg-greenLegacy-700'} opacity-80`} />
              <span className={`relative grid h-[132px] w-[180px] place-items-center rounded-[28px] border shadow-[0_18px_34px_rgb(24_82_55_/_0.14)] ${
                answerVisible ? 'border-white/35 bg-white/16' : 'border-greenLegacy-700/14 bg-white'
              }`}>
                <img
              ...
              <div className="relative z-10 min-w-0">
                ...
                <p className={`mt-3 text-[20px] font-bold leading-relaxed ${answerVisible ? 'text-greenLegacy-50' : 'text-ink'}`}>
                  {answerVisible ? currentCard.rationale : currentCard.text}
                </p>
              </div>
            </div>
          )}
          <div className={`flex min-h-0 flex-col justify-center gap-2 rounded-lg border-2 border-greenLegacy-700/20 bg-white/92 p-2.5 transition-all ${activityReady ? 'animate-slide-in opacity-100' : 'pointer-events-none opacity-0'}`}>
            <button
              type="button"
              ...
              } ${selectedAnswer === 'حوكمة' ? 'border-greenLegacy-700 bg-greenLegacy-700 text-white' : 'border-greenLegacy-700/25 bg-greenLegacy-700/8 text-greenLegacy-800 hover:bg-greenLegacy-700/15'}`}
            >
              حوكمة
            </button>
            <button
              ...
              className={`rounded-lg border-2 px-4 py-3 text-[19px] font-extrabold transition-all disabled:cursor-not-allowed disabled:bg-white disabled:text-ink-muted disabled:shadow-none ${
                phase === 'awaiting-answer' && !narrationLocked ? 'animate-pulse-ring' : ''
              } ${selectedAnswer === 'امتثال' ? 'border-goldLegacy-600 bg-goldLegacy-500 text-white' : 'border-goldLegacy-500/35 bg-goldLegacy-500/10 text-goldLegacy-700 hover:bg-goldLegacy-500/18'}`}
            >
              امتثال
            </button>
    ...
    >
      <div className="flex h-full min-h-0 flex-col px-8 py-3">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[1.05fr_0.95fr] gap-5">
          {scenarioCard && (
            <div className="relative flex min-h-0 flex-col justify-center overflow-visible rounded-[34px] border border-greenLegacy-700/12 bg-white/96 p-4 text-right shadow-[0_22px_55px_rgb(24_82_55_/_0.12)]">
              <span className="pointer-events-none absolute -right-10 -top-10 h-44 w-44 rounded-full bg-tealLegacy-500/12 blur-3xl" />
              <span className="pointer-events-none absolute -left-10 -bottom-10 h-44 w-44 rounded-full bg-goldLegacy-500/14 blur-3xl" />
              <div className="relative z-10 min-h-[164px]">
                ...
                <div className="relative z-10 min-w-0 rounded-[24px] border border-greenLegacy-700/10 bg-white/95 p-3.5 shadow-[0_14px_26px_rgb(24_82_55_/_0.08)]">
                  <p className="text-[16px] font-black text-goldLegacy-700">سيناريو تطبيقي</p>
              ...
              <div className="relative z-10 mt-3 rounded-[22px] border border-goldLegacy-500/24 bg-goldLegacy-500/10 px-4 py-2.5 text-[15px] font-extrabold leading-snug text-brand-strong">
              ...
              return (
                <button
                  ...
                  className={`group flex items-center justify-between gap-4 rounded-[28px] border px-5 py-5 text-right shadow-[0_16px_38px_rgb(24_82_55_/_0.10)] transition-all duration-300 hover:-translate-y-1 ${
                    active
                      ? 'border-greenLegacy-700 bg-greenLegacy-800 text-white'
                      : 'border-greenLegacy-700/12 bg-white/86 text-brand-strong hover:border-goldLegacy-500/45 hover:bg-goldLegacy-50'
                  } ${complete ? 'opacity-70' : ''} ${
                ...
                >
                  <span className={`grid h-14 w-14 shrink-0 place-items-center rounded-2xl ${active ? 'bg-white/16' : 'bg-greenLegacy-700/8'}`}>
                    <Icon name={index === 0 ? 'gavel' : index === 1 ? 'shield' : 'sparkles'} className={`h-8 w-8 ${active ? `text-white ${activeVisualAnimationFor(`${card.title} ${card.text ?? ''}`, index)}` : 'text-greenLegacy-700'}`} />
                  </span>
                  ...
                  <Icon name={active ? 'check' : 'flow'} className={`h-7 w-7 shrink-0 ${active ? `text-tealLegacy-600 ${activeVisualAnimationFor(`${card.title} ${card.text ?? ''}`, index + 3)}` : 'text-goldLegacy-600 group-hover:text-greenLegacy-700'}`} />
                </button>
              );
            })}
          </div>
        ...
        {complete && (
          <div className="pointer-events-none absolute inset-x-[18%] bottom-7 rounded-3xl border border-tealLegacy-500/35 bg-white/88 px-6 py-4 text-center text-[20px] font-extrabold text-brand-strong shadow-card animate-fade-up">
        ...
        {selectedStep != null && currentCard && createPortal(
          <div className="pointer-events-auto absolute inset-0 z-[80] flex items-center justify-center p-8">
            <button
              type="button"
              disabled={narrationLocked}
              className="absolute inset-0 cursor-default bg-greenLegacy-950/34 backdrop-blur-[6px] disabled:cursor-not-allowed"
              aria-label="إغلاق نافذة السيناريو"
              onClick={closeModal}
            />
            <div className="relative z-10 grid h-[600px] w-full max-w-[1120px] grid-cols-[1fr_0.82fr] overflow-visible rounded-[38px] border border-greenLegacy-700/18 bg-white shadow-[0_35px_80px_rgb(0_45_28_/_0.24)] animate-scale-in">
              <button
                type="button"
                disabled={narrationLocked}
                onClick={closeModal}
                className="absolute left-5 top-5 z-20 grid h-11 w-11 place-items-center rounded-full bg-white text-greenLegacy-900 shadow-card ring-1 ring-greenLegacy-700/15 transition hover:bg-greenLegacy-50 disabled:cursor-not-allowed disabled:bg-white disabled:text-ink-muted disabled:shadow-none"
                aria-label="إغلاق"
              >
              ...
              <div className="relative flex min-h-0 flex-col justify-center overflow-visible bg-[linear-gradient(145deg,rgb(247_252_249),rgb(255_255_255)_48%,rgb(244_238_221))] p-9 text-right">
                <span className="pointer-events-none absolute -right-16 -top-16 h-56 w-56 rounded-full bg-greenLegacy-700/10 blur-3xl" />
                <span className="pointer-events-none absolute -left-16 -bottom-16 h-56 w-56 rounded-full bg-goldLegacy-500/14 blur-3xl" />
                <div className="relative z-10">
                  <span className="mb-3 inline-flex items-center gap-2 rounded-full bg-goldLegacy-500/15 px-4 py-1.5 text-[15px] font-extrabold text-goldLegacy-700">
                    محطة {toArabicDigits(selectedStep + 1)}
                  </span>
                  <h3 className="text-[34px] font-black leading-tight text-brand-strong">{currentCard.title}</h3>
                  <p className="mt-4 rounded-[26px] border border-greenLegacy-700/14 bg-white/82 p-5 text-[22px] font-extrabold leading-relaxed text-ink shadow-[0_14px_30px_rgb(24_82_55_/_0.08)]">
                    {scenarioCard?.text}
                  </p>
                  <div className="mt-5 rounded-[26px] border border-goldLegacy-500/30 bg-goldLegacy-500/10 p-5">
                    <p className="mb-2 flex items-center gap-2 text-[16px] font-black text-goldLegacy-700">
                  ...
                  {discussionVisible && (
                    <div className="mt-4 rounded-[26px] border border-tealLegacy-500/28 bg-tealLegacy-500/8 p-5 animate-fade-up">
                      <p className="mb-2 text-[16px] font-black text-tealLegacy-700">مناقشة ناصر</p>
                      <p className="text-[19px] font-bold leading-relaxed text-ink">{currentCard.text}</p>
                    </div>
                  )}
                </div>
              </div>
              <div className="relative flex min-h-0 flex-col items-center justify-center bg-greenLegacy-800 p-8 text-center text-white">
                ...
                <div className="relative z-10 mb-4 grid place-items-center">
                  ...
                  {guidedSpeech.speaking && (
                    <span className="absolute -bottom-1 flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1 text-[13px] font-black text-greenLegacy-800 shadow-card">
                ...
                <div className="relative z-10 mt-7 flex w-full flex-col gap-3">
                  ...
                  {discussionVisible && discussionReady && selectedStep < analysisCards.length - 1 && (
                    <button
                      ...
                      disabled={narrationLocked}
                      onClick={closeModal}
                      className={`rounded-2xl bg-white px-7 py-4 text-[18px] font-black text-greenLegacy-800 shadow-card transition hover:bg-greenLegacy-50 disabled:cursor-not-allowed disabled:bg-white disabled:text-ink-muted disabled:shadow-none ${
                        narrationLocked ? '' : 'animate-pulse-ring'
                      }`}
                    >
                      اختيار محطة أخرى
...
function TitleHead({ slide, showVisual = true }: { slide: Slide; showVisual?: boolean }) {
  return (
    <h2 className={`flex shrink-0 items-center text-[26px] font-extrabold leading-tight text-brand-strong animate-fade-up ${showVisual ? 'gap-2.5' : ''}`}>
      {showVisual && (
        <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-greenLegacy-500/12 to-goldLegacy-500/16 p-1.5 shadow-card">
    ...
    return (
      <StorySlideShell slide={slide} spoken={started ? spoken : 0} showDialogue={showDialogue}>
        ...
        <div className="flex-1 animate-fade-up">
          <span className="chip mb-3 bg-goldLegacy-500/15 text-goldLegacy-600 text-sm font-bold">
        ...
        <div className="hidden">
          <div className="absolute -right-8 -top-8 h-40 w-40 rounded-full bg-greenLegacy-400/18 blur-3xl" />
          <div className="absolute -left-8 -bottom-8 h-40 w-40 rounded-full bg-goldLegacy-400/18 blur-3xl" />
          <div className="hidden">
            <div className="absolute -right-8 -top-8 h-40 w-40 rounded-full bg-greenLegacy-400/18 blur-3xl" />
            <div className="absolute -left-8 -bottom-8 h-40 w-40 rounded-full bg-goldLegacy-400/18 blur-3xl" />
            🎓
  ...
  if (slide.kind === 'reflection' && slide.reflection) {
    return (
      <StorySlideShell slide={slide} spoken={spoken} showDialogue={showDialogue}>
        ...
        <TitleHead slide={slide} />
        <p className="my-4 inline-flex items-center gap-2 rounded-full bg-goldLegacy-500/10 px-4 py-1.5 text-base font-semibold text-goldLegacy-600">
        ...
        <div className="grid w-full max-w-5xl grid-cols-3 gap-3">
          {slide.reflection.map((q, i) => (
            <div key={i} className="rounded-2xl border border-line bg-surface p-4 text-right shadow-card">
              <span className="mb-2 grid h-9 w-9 place-items-center rounded-lg bg-goldLegacy-500/15 text-goldLegacy-600 font-bold tabular">
                {toArabicDigits(i + 1)}
              </span>
