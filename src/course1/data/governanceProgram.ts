...
function cardNarration(card: PptCard, index: number) {
  ...
  const parts = [card.spokenIntro ? withStop(card.spokenIntro) : `${bridge} ${card.title}.`];

  if (card.text) {
    const segments = segmentsOf(card.text);
    if (card.spokenDetail) {
      parts.push(withStop(card.spokenDetail));
...
export const governanceChapterOneSlides = indexSlides([
  ...
  makeSlide({
    ...
    narration: fullNarration(
      ...
      'بهذا التكامل يصبح القرار واضحًا، وتكون المساءلة مرتبطة بصلاحية محددة وليست موزعة بين الجميع.',
    ),
    visual: '🏛️',
    layout: 'pptTitleCards',
  ...
  makeSlide({
    ...
    narration: fullNarration(
      'هنا تتضح الصورة كاملة: الحوكمة تحدد الاتجاه والصلاحيات، والامتثال يتأكد من أن هذا الاتجاه يتحول إلى تطبيق حقيقي.',
      governanceComplianceCards,
      'إذا اجتمعت الحوكمة والامتثال، أصبح لدينا قرار موجه، وتطبيق قابل للقياس، ومنشأة تحافظ على الثقة والاستدامة.',
    ),
    visual: '🔗',
    layout: 'pptThreeColumns',
  ...
  makeSlide({
    ...
    narration:
      'نبدأ الآن نشاط التمييز بين الحوكمة والامتثال. لاحظت لجنة الجودة في مستشفى أن ثلاثين بالمئة من الأطباء لا يوثقون تقارير الخروج وفق السياسة المعتمدة. صنّف كل إجراء أدناه: هل هو قرار حوكمة أم إجراء امتثال؟ ولماذا؟ سنأخذ كل إجراء على حدة، ولن نكشف التصنيف قبل اختيارك. الإجراء الأول: إصدار سياسة جديدة تلزم بالتوثيق الإلكتروني خلال أربع وعشرين ساعة. فكر: هل هذا قرار حوكمة أم إجراء امتثال؟ اختر إجابتك، وبعد ذلك نناقش السبب معًا.',
    visual: '🧠',
...
export const governanceChapterTwoSlides = indexSlides([
  ...
  makeSlide({
    ...
    narration: fullNarration(
      'والآن دورك في التقييم؛ قيّم كل محور من صفر إلى أربعة، واضغط على المحور بعد التفكير لتستمع إلى مناقشتي للنقطة.',
      selfAssessmentCards,
...
export const governanceClosingSlides = indexSlides([
  ...
  makeQuizSlide({
    ...
    narration:
      'وقبل ختام الحقيبة، أمامك الاختبار الختامي الشامل، وهو مختلف عن اختبارات الفصول: ثماني أسئلة موزعة على فصول الحقيبة الثلاثة، تقيس فهمك لرحلة الحوكمة والمخاطر والامتثال كاملة، من بناء الحوكمة إلى إدارة المخاطر. اقرأ كل سؤال بهدوء واختر إجابة واحدة، وراجع التفسير بعد كل سؤال. درجة الاجتياز ستون بالمئة. بالتوفيق.',
